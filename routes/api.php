...
Route::middleware('auth:sanctum')->group(function () {
    ...
    // Shared - bisa diakses admin & siswa, controller otomatis bedakan berdasarkan role
    Route::get('/list-materi', function (\Illuminate\Http\Request $request) {
        if ($request->user()->role === 'admin') {
            return app(AdminMateriController::class)->index($request);
        }
        return app(SiswaMateriController::class)->index($request);
    });
});